...
def gen_fuzz_subdomains(expression, rule):
    ...
    if fuzz_count > 10000000:
        logger.log('ALERT', f'The dictionary generated by this rule is so large：{fuzz_count} > 10000000')
...
class Brute(Module):
    ...
    def main(self, domain):
        ...
        logger.log('INFOR', f'{self.source} module 耗时{self.elapse}秒'
        f'发现{domain}的子域{len(self.subdomains)}个')
        logger.log('DEBUG', f'{self.source} module 发现{domain}的子域:\n'
        f'{self.subdomains}')
    ...
    def run(self):
        ...
        for self.domain in self.domains:
            ...
            logger.log('INFOR', f'结束执行{self.source} module 爆破域名{self.domain}')
            if not self.path:
